class PostsController < ApplicationController
  ...
  def similar_to
    ...
    @posts = ENV["BONSAI_URL"] ? Post.includes(:user).where(id: Post.search(@post.tags.presence || @post.title, size: 4)).where.not(id: @post.id).select_overview_columns.public?.limit(3) : Post.where.not(id: @post.id).last(3)
