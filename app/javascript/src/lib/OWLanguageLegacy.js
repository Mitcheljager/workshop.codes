...
const actions = wordSet(
   ...
   *   yq -jc 'map(.["en-US"]) | sort' config/arrays/wiki/actions.yml | sed 's/","/", "/g' | xclip
   */
...
const values = wordSet(
  /* Regenerate this with (requires [yq](https://github.com/kislyuk/yq)):
   *   yq -jc 'map(.["en-US"]) | sort' config/arrays/wiki/values.yml | sed 's/","/", "/g' | xclip
   */
