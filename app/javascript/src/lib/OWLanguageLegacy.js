...
const actions = wordSet(
  // Regenerate this list with (requires [yq](https://github.com/kislyuk/yq)): yq -jc 'map(.["en-US"]) | sort' config/arrays/wiki/actions.yml | sed 's/","/", "/g' | xclip
...
const values = wordSet(
  // Regenerate this list with (requires [yq](https://github.com/kislyuk/yq)): yq -jc 'map(.["en-US"]) | sort' config/arrays/wiki/values.yml | sed 's/","/", "/g' | xclip
...
const customValues = wordSet(["Mixin", "Constant", "For", "Each"])
const bools = wordSet(["True", "False"])
const operators = "+-/*%=|&<>~^?!"
const customOperators = /\b(?:through|from|to|in(?: steps of)?)\b/
const punc = ":;,.(){}[]+-\\*"
...
function tokenBase(stream, state) {
  ...
  if (stream.match(actionsValuesIdentifier)) {
    ...
    if (values.hasOwnProperty(ident) || customValues.hasOwnProperty(ident)) return "attributeValue"
