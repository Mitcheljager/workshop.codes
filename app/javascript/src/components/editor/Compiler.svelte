...
{#if $selectedLanguages.length && Object.keys($translationKeys).length}
  <div class="dropdown" use:outsideClick on:outsideClick={() => dropdownActive = false}>
    ...
    {#if dropdownActive}
      <div transition:fly={{ duration: 150, y: 20 }} use:escapeable on:escape={() => dropdownActive = false} class="dropdown__content block w-100" style="width: 200px">
        <p class="pl-1/4 pr-1/4 mt-1/8 mb-1/8">Compile for a single language only</p>
