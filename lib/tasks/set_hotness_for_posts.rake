...
task :set_hotness_for_posts => :environment do
  Post.all.each do |post|
    ...
    impressions_count = Ahoy::Event.where(name: "Posts Visit").where("time > ?", 1.week.ago).select { |s| s.properties["controller"] == "posts" && s.properties["action"] == "show" && s.properties["id"] == post.id }.count
